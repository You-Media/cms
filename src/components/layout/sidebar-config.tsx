...
export function buildSidebarNavigation(
  ...
  if (selectedSite === 'editoria' && hasAnyRole(['Publisher', 'AdvertisingManager'])) {
  ...
  // Contatti (solo sito 'editoria', visibile ad Admin e Publisher)
  if (selectedSite === 'editoria' && hasAnyRole(['Admin', 'Publisher'])) {
    items.push({
      name: 'Richieste di Contatto',
      href: APP_ROUTES.DASHBOARD.CONTACTS.LIST,
      icon: (
        <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
