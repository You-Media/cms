...
export default function BannerFormPage({ mode = 'create', bannerId }: { mode?: 'create' | 'edit'; bannerId?: number }) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      try {
        ...
        if (b.model === 'Article') setSelectedArticleTitle(b.model_title || null)
        if (b.model === 'Category') setSelectedCategoryTitle(b.model_title || null)
      } catch (e) {
  ...
  useEffect(() => {
    // no-op placeholder to keep effect list stable
                  ...
                  onDrop={(e) => {
                    ...
                    if (files.length > 0) {
                      ...
                      if (file.type.startsWith('image/')) {
                        ...
                        if (!model || !position) { toast.error('Seleziona prima modello e posizione'); return }
                ...
                <div className="space-y-2">
                  <input
                    ...
                    onChange={async (e) => {
                      ...
                      if (!file) { setImageFile(null); setImagePreview(null); return }
                      if (!model || !position) { toast.error('Seleziona prima modello e posizione'); e.currentTarget.value = ''; return }
                      const req = getRequiredImageFor(model, position as BannerPosition)
                      if (req) {
